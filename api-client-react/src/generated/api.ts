 ...
 */
export const getOpenaiTextToSpeechUrl = () => {
  return `/api/openai/tts`;
};

export const openaiTextToSpeech = async (
  openaiTtsBody: OpenaiTtsBody,
  options?: RequestInit,
): Promise<OpenaiTtsResponse> => {
  return customFetch<OpenaiTtsResponse>(getOpenaiTextToSpeechUrl(), {
    ...options,
    method: "POST",
    headers: { "Content-Type": "application/json", ...options?.headers },
    body: JSON.stringify(openaiTtsBody),
  });
};

export const getOpenaiTextToSpeechMutationOptions = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof openaiTextToSpeech>>,
    TError,
  ...
  TContext
> => {
  const mutationKey = ["openaiTextToSpeech"];
  const { mutation: mutationOptions, request: requestOptions } = options
    ? options.mutation &&
      "mutationKey" in options.mutation &&
      options.mutation.mutationKey
  ...
  > = (props) => {
    ...
    return openaiTextToSpeech(data, requestOptions);
  };

  return { mutationFn, ...mutationOptions };
};

export type OpenaiTextToSpeechMutationResult = NonNullable<
  Awaited<ReturnType<typeof openaiTextToSpeech>>
>;
 ...
 * @summary Convert text to speech audio
 */
export const useOpenaiTextToSpeech = <
  TError = ErrorType<unknown>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
  ...
  TContext
> => {
  return useMutation(getOpenaiTextToSpeechMutationOptions(options));
};
